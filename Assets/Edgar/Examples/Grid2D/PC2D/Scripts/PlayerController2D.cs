...
namespace Edgar.Unity.Examples.PC2D
{
    ...
    public class PlayerController2D : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (UnityEngine.Input.GetButtonDown(Input.JUMP))
            {
                _motor.Jump();
                _motor.DisableRestrictedArea();
            }

            _motor.jumpingHeld = UnityEngine.Input.GetButton(Input.JUMP);

            // XY freedom movement
            if (_motor.motorState == PlatformerMotor2D.MotorState.FreedomState)
            {
                _motor.normalizedXMovement = UnityEngine.Input.GetAxis(Input.HORIZONTAL);
                _motor.normalizedYMovement = UnityEngine.Input.GetAxis(Input.VERTICAL);

                return; // do nothing more
            }

            // X axis movement
            if (Mathf.Abs(UnityEngine.Input.GetAxis(Input.HORIZONTAL)) > Globals.INPUT_THRESHOLD)
            {
                _motor.normalizedXMovement = UnityEngine.Input.GetAxis(Input.HORIZONTAL);
            }
            else
            ...
            if (UnityEngine.Input.GetAxis(Input.VERTICAL) != 0)
            {
                bool up_pressed = UnityEngine.Input.GetAxis(Input.VERTICAL) > 0;
            ...
            else if (UnityEngine.Input.GetAxis(Input.VERTICAL) < -Globals.FAST_FALL_THRESHOLD)
            {
                _motor.fallFast = false;
            }

            if (UnityEngine.Input.GetButtonDown(Input.DASH))
            {
                _motor.Dash();
            }
        }
